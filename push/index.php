...
function sendPushNotification($registration_ids, $message) {
    ...
    define('GOOGLE_API_KEY', 'AIzaSyBazPLagKUp7xHsyPriEzOIQ-jGA9rGja4');
...
if(!empty($_GET['push'])) {
    ...
    $query = "SELECT device_id FROM register";
    if($query_run = mysql_query($query)) {

        $gcmRegIds = array();
        while($query_row = mysql_fetch_assoc($query_run)) {

            array_push($gcmRegIds, $query_row['device_id']);
...
if(!empty($_GET['shareRegId'])) {
    ...
    $gcmRegId = $_POST['device_id'];
    $query = "INSERT INTO register VALUES ('', '$gcmRegId')";
    if($query_run = mysql_query($query)) {
        echo 'OK';
        exit;
    }   
